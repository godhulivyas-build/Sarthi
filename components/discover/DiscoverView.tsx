  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-['Lexend'] min-h-screen pb-24">
      ...
      <main className="max-w-2xl mx-auto p-4 space-y-4">
        <div className="grid grid-cols-2 gap-2 bg-emerald-100/60 dark:bg-slate-800 p-1.5 rounded-2xl">
          <button
          ...
          >
            {tt('I want to Sell', 'मुझे बेचना है')}
          </button>
          <button
            onClick={() => setMode('buy')}
            className={`py-3 rounded-xl font-bold text-sm transition-colors ${
          ...
          >
            {tt('I want to Buy', 'मुझे खरीदना है')}
          </button>
        </div>

        {mode === 'sell' ? <SellFlow /> : <BuyFlow />}
      </main>
    </div>
  );
};
